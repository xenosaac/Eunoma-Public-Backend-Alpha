...
const resourceType = `${bridgePackage}::eunoma_bridge::PendingWithdrawPayloadsV2`;
const existing = await getResource(aptosNodeUrl, bridgePackage, resourceType);
if (existing.status === 200) {
  console.log("PendingWithdrawPayloadsV2 already initialized");
  process.exit(0);
}
...
const cliArgs = [
  ...
  "run",
  "--function-id",
  `${bridgePackage}::eunoma_bridge::init_pending_withdraw_payloads_v2`,
];
...
console.log("PendingWithdrawPayloadsV2 initialized");
