...
const resourceType = `${bridgePackage}::eunoma_bridge::PendingWithdrawAttestationsV2`;
const existing = await getResource(aptosNodeUrl, bridgePackage, resourceType);
if (existing.status === 200) {
  console.log("PendingWithdrawAttestationsV2 already initialized");
  process.exit(0);
}
...
const cliArgs = [
  ...
  "run",
  "--function-id",
  `${bridgePackage}::eunoma_bridge::init_pending_withdraw_attestations_v2`,
];
...
console.log("PendingWithdrawAttestationsV2 initialized");
