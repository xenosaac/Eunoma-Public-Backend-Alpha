...
const resourceType = `${bridgePackage}::eunoma_bridge::PendingDepositBindingsV2`;
const existing = await getResource(aptosNodeUrl, bridgePackage, resourceType);
if (existing.status === 200) {
  console.log("PendingDepositBindingsV2 already initialized");
  process.exit(0);
}
...
const cliArgs = [
  ...
  "run",
  "--function-id",
  `${bridgePackage}::eunoma_bridge::init_pending_deposit_bindings_v2`,
];
...
console.log("PendingDepositBindingsV2 initialized");
