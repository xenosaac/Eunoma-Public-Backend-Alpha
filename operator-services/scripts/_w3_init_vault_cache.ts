...
async function main() {
  ...
  if (!r.success) throw new Error(`init_vault_config_cache failed: ${r.vm_status}`);

  updateTargetDeploy((d) => {
    d.publishes ??= {};
    d.publishes.init_vault_config_cache = {
      tx: r.hash, gas_used: Number(r.gas_used), gas_unit_price: Number(r.gas_unit_price ?? 100),
    };
  });
  console.log(`[state] deploys.${DEPLOY_ID}.publishes.init_vault_config_cache written`);
}
