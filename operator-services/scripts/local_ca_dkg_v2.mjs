...
if (!plan.caDkgV2Roster || !plan.caDkgV2RosterHash) {
  console.error("local cluster config is missing ca_dkg_v2 roster. Re-run `npm run local:cluster:config -- --force`.");
  process.exit(2);
}
...
const requestId = process.env.REQUEST_ID ?? `ca-dkg-v2-${Date.now()}`;
const sessionId =
  process.env.SESSION_ID ??
  `${plan.caDkgV2Roster.operatorSetVersion}:${plan.caDkgV2Roster.dkgEpoch}:${plan.caDkgV2RosterHash}`;

const res = await fetch(new URL("/v2/dkg/ca/v2/start", coordinatorUrl), {
  method: "POST",
  headers: {
...
const artifactDir = resolve(stateRoot, "cluster/dkg");
  ...
  `${JSON.stringify(
    {
      ...
      note: "ca_dkg_v2 completed without deriving or persisting vaultEk",
    },
    null,
    2,
  )}\n`,
