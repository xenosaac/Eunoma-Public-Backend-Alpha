...
const resourceType = `${bridgePackage}::eunoma_bridge::PendingWithdrawProofsV2`;
const existing = await getResource(aptosNodeUrl, bridgePackage, resourceType);
if (existing.status === 200) {
  console.log("PendingWithdrawProofsV2 already initialized");
  process.exit(0);
}
...
const cliArgs = [
  ...
  "run",
  "--function-id",
  `${bridgePackage}::eunoma_bridge::init_pending_withdraw_proofs_v2`,
];
...
console.log("PendingWithdrawProofsV2 initialized");
