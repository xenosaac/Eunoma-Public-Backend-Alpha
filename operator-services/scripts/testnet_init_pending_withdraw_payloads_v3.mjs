...
const resourceType = `${bridgePackage}::eunoma_bridge::PendingWithdrawPayloadsV3`;
const existing = await getResource(aptosNodeUrl, bridgePackage, resourceType);
if (existing.status === 200) {
  console.log("PendingWithdrawPayloadsV3 already initialized");
  process.exit(0);
}
...
const cliArgs = [
  ...
  "run",
  "--function-id",
  `${bridgePackage}::eunoma_bridge::init_pending_withdraw_payloads_v3`,
];
...
console.log("PendingWithdrawPayloadsV3 initialized");
